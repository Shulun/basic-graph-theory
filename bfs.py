...
# Return the distance of the path between two nodes
def path(G, v1, v2):
    distance_from_start = {}
    open_list = [v1]
    distance_from_start[v1] = 0
    # distance_frim_start[v1] = [v1] # start of the actual path
    while len(open_list) > 0:
        current = open_list.pop(0)
        for neighbor in G[current].keys():
            if neighbor not in distance_from_start:
                distance_from_start[neighbor] = distance_from_start[current] + 1
                # distance_from_start[neighbor] = distance_from_start[current] + [neighbor] # return the actual path
